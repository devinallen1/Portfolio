...
function challenge (chance) 
{
    ...
    if(inChallenge){
        ...
        for(var i = 0; i < len; i++){
            playerArray[playerTurn].getHand().push(Deck.dealCard());
        }
        inChallenge = false;
...
function slap (slapperSock, hand)
{
	if (isSlappable == true)
	{
        Deck.getHand().forEach(c => 
            playerArray.filter(hnd => {return hnd.socket == slapperSock}).addTopCard(c));
        isSlappable = false;
	}
	...
	{
        for(let i = 0; i < 2; i++){
        Deck.push(playerArray.filter(hnd => {return hnd.socket == slapperSock;}).getHand()[0]);
        }
	}
}
